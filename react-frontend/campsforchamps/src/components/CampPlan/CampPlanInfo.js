...
export const CampPlanInfo = (props) => {
  ...
  try {
    ...
    const removeFromCartFunc = () => {
      if (isAuthenticated()) {
        removeItemFromCart("plan", plan.id);
        setMessage({
          ...message,
          display: true,
    ...
    return (
      <div>
        ...
        <section className="camp-plan-info">
          <Link to="/camp-plan">
            <i className="fas fa-arrow-left"></i> Go back
          </Link>

          <div className="img-container">
            <img src={plan.image ? plan.image : ""} alt={plan.name} />
          </div>
          ...
          <div className="extra-info">
            <div className="dates">
              <i className="fad fa-calendar-star"></i> Journey starts at{" "}
              <em>{moment(plan.start_date).format("MMM Do YYYY")}</em> ends at{" "}
              <em>{moment(plan.end_date).format("MMM Do YYYY")}</em>
            </div>
